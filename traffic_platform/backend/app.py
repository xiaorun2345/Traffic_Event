...
VIDEO_PID_FILE = RUNTIME_ROOT / "video.pid"
...
def ensure_runtime():
    for path in (BACKUP_ROOT, HLS_ROOT, LOG_ROOT):
...
def video_pid():
    if not VIDEO_PID_FILE.exists():
        return None
    try:
        pid = int(VIDEO_PID_FILE.read_text().strip())
    except Exception:
        return None
...
def start_video():
    ...
    for item in HLS_ROOT.glob("*"):
        if item.is_file():
            item.unlink()
    log_path = LOG_ROOT / "ffmpeg-hls.log"
    cmd = [
        "ffmpeg",
        "-hide_banner",
        "-y",
    ...
    if source.startswith("rtsp://"):
        transport = str(config.get("RtspTransport", "tcp") or "tcp")
    ...
    cmd += [
        ...
        str(HLS_ROOT / "live.m3u8"),
    ]
    log = open(log_path, "ab")
    proc = subprocess.Popen(cmd, cwd=str(OUTPUT_DIR), stdout=log, stderr=subprocess.STDOUT)
    VIDEO_PID_FILE.write_text(str(proc.pid), encoding="utf-8")
    return {"started": True, "pid": proc.pid, "playlist": "/hls/live.m3u8", "log": str(log_path.relative_to(PROJECT_ROOT))}


def stop_video():
    pid = video_pid()
    if not pid:
        return {"stopped": False, "message": "video stream is not running"}
    terminate_pid(pid)
    if VIDEO_PID_FILE.exists():
        VIDEO_PID_FILE.unlink()
    return {"stopped": True, "pid": pid}
...
class PlatformHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            if path.startswith("/hls/"):
                self.serve_file(HLS_ROOT, path[len("/hls/"):], "application/vnd.apple.mpegurl")
                return
            if path == "/":
                path = "/index.html"
    ...
    def handle_api_get(self, path):
        if path == "/api/status":
            ...
            self.send_json(*response_payload(video_status()))
    ...
    def handle_api_post(self, path, body):
        if path == "/api/main-config":
            ...
            self.send_json(*response_payload(start_video()))
